class User < ActiveRecord::Base
  ...
  def stats
    return {
      ...
      likes: pages.count,
      results: games.order("updated_at").map{|g| [g.updated_at, g.results.where(correct: true).count]}
    }
  end
  ...
  def self.stats
    return {
      ...
      results: Game.order("updated_at").last(100).map{|g| [g.updated_at, g.results.where(correct: true).count]}
    }
  end
